...
- **LABELフィールド**: LABELフィールドのフィールドコード・フィールド名を「未定義」で表示
...
### LABELフィールド機能
- **フィールドコード**: LABELフィールドのフィールドコードを「未定義」で表示
- **フィールド名**: LABELフィールドのフィールド名を「未定義」で表示
...
### 4. LABELフィールド表示設定
- **スキーマ表示**: チェックボックスをONにするとLABELフィールドをスキーマに表示、OFFにすると非表示
- **レコード表示**: LABELフィールドは常に除外（データではないため）
- **設定の確認**: コンソールログでフィルタリング結果を確認可能

### 5. CSV出力機能
- **スキーマCSV**: データベース構造をCSV形式でダウンロード
- **レコードCSV**: 取得したレコードデータをCSV形式でダウンロード
- **サブテーブル含むCSV**: サブテーブルデータも含む完全なCSV出力
- ファイル名: `アプリ名_schema_YYYY-MM-DD.csv` / `アプリ名_records_YYYY-MM-DD.csv` / `アプリ名_full_records_YYYY-MM-DD.csv`
...
- **v1.14.0**: CSV出力でのHTMLタグ保持
  - CSV出力時にリッチエディターフィールドの初期値からHTMLタグを除去しないように変更
  - HTMLタグを含む初期値はそのままCSVに出力されるように修正
  - ユーザーが必要に応じて後処理できるよう、元のHTML形式を保持
- **v1.13.0**: システムフィールド表示の修正
  - 更新者、作成者フィールドの適切な表示対応
  - システムフィールドのオブジェクト形式データを適切に処理
  - UI表示とCSV出力両方でシステムフィールド対応
- **v1.12.0**: オプション詳細表示のプロパティ名ボールド体化
  - オプション詳細のプロパティ名をボールド体で表示
  - UI表示での可読性をさらに向上
  - HTML版とテキスト版の関数を分離（UI表示用とCSV出力用）
- **v1.11.0**: オプション詳細表示の可読性向上
  - オプション詳細のプロパティ表示を改行区切りに変更（セミコロン区切りから）
  - UI表示での可読性を大幅に向上
  - CSV出力では従来通りセミコロン区切りを維持（CSVファイルの性質上）
- **v1.10.0**: LABELフィールドのレコード表示除外機能
  - LABELフィールドをレコード表示から常に除外（データではないため）
  - スキーマ表示では従来通りチェックボックスで表示/非表示を制御
  - レコード表示、CSV出力、サブテーブル表示すべてでLABELフィールドを除外
  - より適切なデータ表示の実現
- **v1.9.0**: LABELフィールド表示スイッチと関連レコード一覧条件表示の改善
  - LABELフィールドの表示/非表示を切り替え可能なスイッチを追加
  - 関連レコード一覧の条件とソート条件表示を改善（field・relatedField対応）
  - LABELフィールドのフィールドコード・フィールド名を「未定義」で表示
  - レイアウトからLABELフィールドを収集しスキーマに追加する機能
  - 全モジュール（UI表示、CSV出力、グループ処理、サブテーブル処理）での統一対応
- **v1.8.0**: UI表示の最適化とCSVヘッダーの統一
  - 画面表示から説明カラムを削除し、より見やすい表示に最適化
  - CSV出力のヘッダーを画面表示に完全統一
  - メインフィールド、グループフィールド、サブテーブルフィールドの全CSV出力で統一対応
  - シンプルで一貫性のあるUI/UXの実現
...
- **表示省略**: LABELフィールドの実際のフィールドコードやラベル名はhtmlタグが混在し冗長になるので「未定義」として表示します
